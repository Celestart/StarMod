package fr.circus.celestart.Gui;
...
public class GuiCustomMainMenu extends GuiScreen {
	...
	/**
	 * A random number between 0.0 and 1.0, used to determine if the title
	 * screen says <a href=
	 * "https://minecraft.gamepedia.com/Menu_screen#Minceraft">Minceraft</a>
	 * instead of Minecraft. Set during construction; if the value is less than
	 * .0001, then Minceraft is displayed.
	 */
	...
	private static final ResourceLocation SPLASH_TEXTS = new ResourceLocation(Reference.MOD_ID, "textures/texts/splashes.txt");
	private static final ResourceLocation MINECRAFT_TITLE_TEXTURES = new ResourceLocation(Reference.MOD_ID,
			"textures/gui/Celestart.png");
	private static final ResourceLocation field_194400_H = new ResourceLocation(Reference.MOD_ID, "textures/gui/edition.png");
	...
	/**
	 * A screen generated by realms for notifications; drawn in adition to the
	 * main menu (buttons and such from both are drawn at the same time). May be
	 * null.
	 */
	...
	public GuiCustomMainMenu() {
		...
		new Thread()
        {
            ...
            public void run()
            {
                try
                {
                    URL url = new URL("http://149.91.88.169/InfosServers/Infos.txt");
	...
	/**
	 * Is there currently a realms notification screen, and are realms
	...
	private void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_) {
		 ...
		 p_73969_1_, 98, 20, I18n.format("menu.singleplayer")));
		 /* this.buttonList.add(new GuiButton(2, this.width / 2 - 100, p_73969_1_
		 * + p_73969_2_ * 1, I18n.format("menu.multiplayer")));
		 * this.realmsButton = this.addButton(new GuiButton(14, this.width / 2 +
		 * 2, p_73969_1_ + p_73969_2_ * 2, 98, 20,
		 * I18n.format("menu.online").replace("Minecraft", "").trim()));
		 * this.buttonList.add(modButton = new GuiButton(6, this.width / 2 -
		 * 100, p_73969_1_ + p_73969_2_ * 2, 98, 20,
		 * I18n.format("fml.menu.mods")));
		 */

		this.buttonList.add(new GuiButton(40, this.width / 2 - 100, this.height / 8 - 6, "Celestart"));
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 40) {
			FMLClientHandler.instance().connectToServer(this, new ServerData("Celestart", "play.celestart.fr", false));
		}
	}
	 ...
	 * bufferbuilder = tessellator.getBuffer(); GlStateManager.matrixMode(5889);
	 * GlStateManager.pushMatrix(); GlStateManager.loadIdentity();
	 * Project.gluPerspective(120.0F, 1.0F, 0.05F, 10.0F);
	 * GlStateManager.matrixMode(5888); GlStateManager.pushMatrix();
	 * GlStateManager.loadIdentity(); GlStateManager.color(1.0F, 1.0F, 1.0F,
	 * 1.0F); GlStateManager.rotate(180.0F, 1.0F, 0.0F, 0.0F);
	 ...
	 * 8;
	 * 
	 * for (int j = 0; j < 64; ++j) { GlStateManager.pushMatrix(); float f =
	 * ((float)(j % 8) / 8.0F - 0.5F) / 64.0F; float f1 = ((float)(j / 8) / 8.0F
	 * - 0.5F) / 64.0F; float f2 = 0.0F; GlStateManager.translate(f, f1, 0.0F);
	 * GlStateManager.rotate(MathHelper.sin(this.panoramaTimer / 400.0F) * 25.0F
	 * + 20.0F, 1.0F, 0.0F, 0.0F); GlStateManager.rotate(-this.panoramaTimer *
	 * 0.1F, 0.0F, 1.0F, 0.0F);
	 * 
	 * for (int k = 0; k < 6; ++k) { GlStateManager.pushMatrix();
	 * 
	 ...
	 * bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR); int l =
	 * 255 / (j + 1); float f3 = 0.0F; bufferbuilder.pos(-1.0D, -1.0D,
	 * 1.0D).tex(0.0D, 0.0D).color(255, 255, 255, l).endVertex();
	 * bufferbuilder.pos(1.0D, -1.0D, 1.0D).tex(1.0D, 0.0D).color(255, 255, 255,
	 * l).endVertex(); bufferbuilder.pos(1.0D, 1.0D, 1.0D).tex(1.0D,
	 * 1.0D).color(255, 255, 255, l).endVertex(); bufferbuilder.pos(-1.0D, 1.0D,
	 * 1.0D).tex(0.0D, 1.0D).color(255, 255, 255, l).endVertex();
	 * tessellator.draw(); GlStateManager.popMatrix(); }
	 * 
	 * GlStateManager.popMatrix(); GlStateManager.colorMask(true, true, true,
	 * false); }
	 * 
	 ...
	 * GlStateManager.depthMask(true); GlStateManager.enableCull();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		Gui.drawScaledCustomSizeModalRect(this.width / 2 - 103, 54, 0, 0, 1, 1, 205, 107, 1, 1);
		...
		this.drawString(this.fontRenderer, "Copyright Mojang AB. Do not distribute!", this.widthCopyrightRest,
				this.height - 10, -1);

		if (mouseX > this.widthCopyrightRest && mouseX < this.widthCopyrightRest + this.widthCopyright
				&& mouseY > this.height - 10 && mouseY < this.height && Mouse.isInsideWindow()) {
			drawRect(this.widthCopyrightRest, this.height - 1, this.widthCopyrightRest + this.widthCopyright,
					this.height, -1);
		}
		...
		if (this.openGLWarning1 != null && !this.openGLWarning1.isEmpty()) {
			...
			this.drawString(this.fontRenderer, this.openGLWarning2, (this.width - this.openGLWarning2Width) / 2,
